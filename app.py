...
def send_sms():
    ...
    print(f"Selected Secretariat Member: {selected_secretariat_member}")  # Debugging 
    recipients = []

    for group in selected_groups:
        if group == 'Secretariat' and selected_secretariat_member:

            secretariat_df = datasets['Secretariat']
            secretariat_member = secretariat_df[secretariat_df['First Name'] == selected_secretariat_member.split(' ')[0]]
            recipients.extend(secretariat_member['Phone Number'].tolist())
        else:
